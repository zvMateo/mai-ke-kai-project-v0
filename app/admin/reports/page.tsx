...
async function getReportStats() {
  ...
  const monthlyRevenue = bookings?.reduce((sum, b) => sum + Number(b.paid_amount), 0) || 0
  ...
  const servicesRevenue =
    serviceBookings?.reduce((sum, s) => sum + Number(s.price_at_booking) * (s.quantity || 1), 0) || 0

  // Occupancy rate (beds occupied / total beds)
  const totalBeds = 18
  const { data: occupiedBeds } = await supabase.from("bookings").select("guests_count").eq("status", "checked_in")

  const currentOccupancy = occupiedBeds?.reduce((sum, b) => sum + (b.guests_count || 0), 0) || 0
  ...
  const lastMonthRevenue = lastMonthBookings?.reduce((sum, b) => sum + Number(b.paid_amount), 0) || 0
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Track revenue, occupancy, and service performance</p>
        </div>
        <DateRangePicker />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Accommodation</CardTitle>
            <div className="w-10 h-10 bg-seafoam/20 rounded-lg flex items-center justify-center">
              <Bed className="w-5 h-5 text-seafoam" />
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold">${stats.accommodationRevenue.toLocaleString()}</p>
            <p className="text-xs text-muted-foreground mt-1">Room & bed revenue</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">${stats.servicesRevenue.toLocaleString()}</p>
            <p className="text-xs text-muted-foreground mt-1">Surf, tours & transport</p>
          </CardContent>
        </Card>
