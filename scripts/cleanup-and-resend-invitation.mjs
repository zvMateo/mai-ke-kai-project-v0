#!/usr/bin/env node
/**
 * Script to clean up and resend admin invitation
 * Run: node scripts/cleanup-and-resend-invitation.mjs
 */
...
async function cleanupAndResend() {
  console.log("Starting cleanup and resend process...\n");

  // Step 1: Delete all invitations for this email
  console.log("Step 1: Deleting existing invitations...");
  ...
  if (deleteInviteError) {
    console.log("No existing invitations to delete or:", deleteInviteError.message);
  } else {
    console.log("✓ Existing invitations deleted");
  }

  // Step 2: Check if user exists in auth and delete if needed
  ...
  try {
    ...
    console.log("Error checking auth users (this is OK if user doesn't exist):", err.message);
  }

  // Step 3: Check if user exists in public.users table and delete if needed
  console.log("\nStep 3: Checking for existing user in public.users...");
  ...
  if (selectError && selectError.code !== "PGRST116") {
    ...
    console.log(`Found existing public user: ${existingPublicUser.id}`);
    console.log("Deleting existing public user...");
    ...
    if (deletePublicError) {
      console.error("Error deleting public user:", deletePublicError);
    } else {
      console.log("✓ Existing public user deleted");
    }
  } else {
    console.log("No existing public user found");
  }

  // Step 4: Generate new token and expiry
  console.log("\nStep 4: Creating new invitation...");
  ...
  console.log("\n========================================");
  console.log("CLEANUP AND RESEND COMPLETE!");
  console.log("========================================");
  console.log(`\nEmail: ${ADMIN_EMAIL}`);
  console.log(`Expires: ${expiresAt.toLocaleString()}`);
  console.log(`\nURL: ${invitationUrl}`);
  console.log("\nPlease use this new link to accept the invitation.");
}
...
cleanupAndResend().catch(console.error);
